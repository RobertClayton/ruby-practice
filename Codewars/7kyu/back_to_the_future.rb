...
def back_to_the_future(str)
  ...
  case input[2]
  	when "January"
    	return "I'm leaving here on #{input[0]} #{input[1]} October!"
    when "April"
    	return "I'm leaving here on #{input[0]} #{input[1]} July!"
    when "September"
    	return "I'm leaving here on #{input[0]} #{input[1]} December!"
    when "February" #28
   		return "I'm leaving here on #{input[0]} #{input[1]} March!"
    when "March"#31-30
    	if input[1].to_i > 30
      	return "Doc, I can't get back to the future!"
      else
      	return "I'm leaving here on #{input[0]} #{input[1]} November!"
      end
  	else
      return "Doc, I can't get back to the future!"
  end
